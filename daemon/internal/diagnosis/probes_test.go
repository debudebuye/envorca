...
func TestRegistryRegistersAllComponents(t *testing.T) {
	t.Setenv("RUNORKA_WSL_EXE", "/bin/echo")
...
func TestWSLProbeBootFailureIsCriticalAndRepairable(t *testing.T) {
	...
	runner := fakeRunner(map[string]func() error{
		"--distribution Ubuntu-24.04 -- echo runorka:boot": func() error { return context.DeadlineExceeded },
	})
	st := statusFor(t, "wsl", runner)
	if st.Status != runorkav1.Status_CRITICAL {
...
func TestWSLProbeNoDefaultDistroWarns(t *testing.T) {
	...
	st := statusFor(t, "wsl", runner)
	if st.Status != runorkav1.Status_WARNING {
...
func TestDaemonAndResourcesHealthy(t *testing.T) {
	...
	if st := statusFor(t, "daemon", runner); st.Status != runorkav1.Status_HEALTHY {
		t.Errorf("daemon = %s, want HEALTHY", st.Status)
	}
	if st := statusFor(t, "resources", runner); st.Status != runorkav1.Status_HEALTHY {
		t.Errorf("resources = %s, want HEALTHY", st.Status)
	}
}

func statusFor(t *testing.T, id string, runner wsl.Runner) *runorkav1.ComponentStatus {
	t.Helper()
