...
func components(st *runorkav1.ComponentStatus) []*runorkav1.ComponentStatus {
	return []*runorkav1.ComponentStatus{st}
}

func wslStatus(s runorkav1.Status, summary string) *runorkav1.ComponentStatus {
	return &runorkav1.ComponentStatus{Id: "wsl", Status: s, Summary: summary}
}
...
func TestPlanEmptyWhenHealthy(t *testing.T) {
	r := New(nil)
	plan := r.Plan(components(wslStatus(runorkav1.Status_HEALTHY, "WSL2 ready")))
	if len(plan) != 0 {
...
func TestPlanOffersStartAndRestartOnBootFailure(t *testing.T) {
	r := New(nil)
	st := wslStatus(runorkav1.Status_CRITICAL, "Linux environment failed to start")
...
func TestPlanOffersOnlyRestartOnQueryFailure(t *testing.T) {
	r := New(nil)
	st := wslStatus(runorkav1.Status_CRITICAL, "could not query WSL status")
	plan := r.Plan(components(st))
...
func TestPlanEmptyForWarnings(t *testing.T) {
	r := New(nil)
	plan := r.Plan(components(wslStatus(runorkav1.Status_WARNING, "no default Linux distribution")))
	if len(plan) != 0 {
...
func TestExecuteStartVerifies(t *testing.T) {
	...
	outcome, err := r.Execute(context.Background(), "wsl.start", false,
		components(wslStatus(runorkav1.Status_CRITICAL, "Linux environment failed to start")))
	if err != nil {
...
func TestExecuteRestartConfirmed(t *testing.T) {
	...
	outcome, err := r.Execute(context.Background(), "wsl.restart", true,
		components(wslStatus(runorkav1.Status_CRITICAL, "could not query WSL status")))
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if !outcome.Success {
...
func TestExecuteRejectsNoLongerApplicable(t *testing.T) {
	...
	r := New(s.run)
	outcome, err := r.Execute(context.Background(), "wsl.start", false,
		components(wslStatus(runorkav1.Status_HEALTHY, "WSL2 ready")))
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if outcome.Success {
